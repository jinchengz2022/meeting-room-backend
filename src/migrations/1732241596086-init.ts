...
export class Init1732241596086 implements MigrationInterface {
    name = 'Init1732241596086'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`meeting_room\` (\`id\` int NOT NULL AUTO_INCREMENT COMMENT '会议室id', \`name\` varchar(50) NOT NULL COMMENT '会议室名称', \`capacity\` int NOT NULL COMMENT '会议室容量', \`position\` varchar(255) NOT NULL COMMENT '会议室位置', \`desc\` varchar(100) NOT NULL COMMENT '描述' DEFAULT '', \`isBooked\` tinyint NOT NULL COMMENT '是否被预定' DEFAULT 0, \`createTime\` datetime(6) NOT NULL COMMENT '创建时间' DEFAULT CURRENT_TIMESTAMP(6), \`updateTime\` datetime(6) NOT NULL COMMENT '更新时间' DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`booking\` (\`id\` int NOT NULL AUTO_INCREMENT COMMENT '预定id', \`startTime\` datetime(6) NOT NULL COMMENT '会议开始时间' DEFAULT CURRENT_TIMESTAMP(6), \`endTime\` datetime(6) NOT NULL COMMENT '会议结束时间' DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`status\` varchar(255) NOT NULL COMMENT '会议室状态' DEFAULT 'applying', \`note\` varchar(255) NOT NULL COMMENT '备注' DEFAULT '', \`people\` text NOT NULL, \`userId\` int NULL, \`roomId\` int NULL COMMENT '会议室id', PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
