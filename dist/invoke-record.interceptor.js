...
let InvokeRecordInterceptor = InvokeRecordInterceptor_1 = class InvokeRecordInterceptor {
    ...
    intercept(context, next) {
        ...
        this.logger.debug(`${ip} ${path} ${method} ${userAgent}: ${context.getClass().name} ${context.getHandler().name} invoke...`);
        this.logger.debug(`user: ${request?.user?.userId} ${request?.user?.username}`);
        const now = Date.now();
        return next.handle().pipe((0, rxjs_1.tap)((res) => {
            this.logger.debug(`${ip} ${path} ${method} ${userAgent}: ${response.statusCode}: ${Date.now() - now}ms`);
